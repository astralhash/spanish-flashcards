...
(function () {
  ...
  /* drop a leading definite article "the" from an English gloss,
     never down to empty ("the" alone stays "the") */
  function stripThe(s) {
    var r = s.replace(EN_THE, '');
    return r.length ? r : s;
  }
  ...
  /* both article conventions are optional on either side — Spanish
     el/la/un/... as well as an English "the" (glosses vary) */
  function stripArt(s) {
    s = s.replace(ES_ART, '').replace(EN_THE, '');
    return s.replace(ES_ART, '');
  }

  function baseMatch(t, g) {
    if (t === g) return true;
    var ta = stripArt(t), ga = stripArt(g);
    var va = [t, ta, stripThe(ta)],
        vb = [g, ga, stripThe(ga)];
    if (!ta.length || !ga.length) return false;
    /* a leading infinitive "to" is optional on either side */
    var i, j;
    for (i = 0; i < va.length; i++) {
      for (j = 0; j < vb.length; j++) {
        if (va[i] === vb[j]) return true;
        if (stripTo(va[i]) === stripTo(vb[j])) return true;
      }
    }
    return false;
  }
